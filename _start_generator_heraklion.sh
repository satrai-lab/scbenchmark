...
print_loading_animation "Starting the synthetic data generator for Heraklion" &
animation_pid=$!
...
#            print_green "Generator [Smart City Bus] Initiated!"
#            python3 0_all_in_one_generate.py >/dev/null 2>&1 &
#            #python3 0_all_in_one_generate.py
#        else
#            stop_animation=true
#            kill -9 "$animation_pid" &>/dev/null
#	    echo -e "\n" # Print a newline to clear the line
...
#    fi
#
    cd ../generator_smart_city_heraklion/ >/dev/null 2>&1
   # Check if the directory change was successful
   if [ $? -eq 0 ]; then
       # Run the Python script and suppress output/errors
