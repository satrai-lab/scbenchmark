...
print_loading_animation "Starting the synthetic data generator for Kerkira" &
animation_pid=$!

# Check if the Docker container is already running
if ps aux | grep -q "orionld"; then
    # Change directory and suppress output/errors
    cd generator_scb_kerkira >/dev/null 2>&1
    ...
    cd ../generator_smart_city_kerkira/ >/dev/null 2>&1
   # Check if the directory change was successful
   if [ $? -eq 0 ]; then
       # Run the Python script and suppress output/errors
